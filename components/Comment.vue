...
export default {
  ...
  mounted() {
    ...
    if (info) {
      for (const i of ['name', 'email', 'image'])
  ...
  <div class="comment">
    <section class="comment-section">
      <div class="comment-form">
        <div class="input-box">
          <input
            v-for="(val, key, index) in { name: 'Name', email: 'Email' }" :key="index" v-model="form[key]"
            type="text" :placeholder="val"
          >
        </div>
        <div v-if="isReply" class="reply-name">
          <span class="">@{{ replyObj.reply_name }}</span>
          <span class="iconfont icon-close" @click="cancel" />
        </div>

        <textarea v-model="form.content" class="textarea" placeholder="What do you want to say..." />

        <!-- submit button and loading -->
        <div class="bottom">
          <button
            id="comment" type="button" :class="status == 9 ? 'active' : ''"
            @click="submitVerify"
          >
            SUBMIT
          </button>
          ...
          <template v-if="status == 7">
            <div class="hint loading">
              <div class="sk-circle selected">
                <div v-for="item in 12" :key="item" class="sk-child" :class="`sk-circle${item}`" />
              </div>
      ...
      <template v-if="comment.total > 0">
        ...
        <div class="comment-list">
          <transition-group name="comment-item">
            <!-- Comment List -->
            <div v-for="item in comment.data" :key="item.id" class="comment-item" :data-id="item.id">
              <div class="comment-item-box">
                <div class="head">
                  ...
                  <div class="name">
                    <a :class="{ 'admin-mark': item.admin }">{{ item.name }}<span
                      v-if="item.admin"
                      class="mark-tit"
                    >这里有我，并且不止有我</span></a>
                    <div class="r">
                      <div class="reply" @click="reply(item, 1)">
                        reply
                      </div>
              ...
              <!-- sub-comment to list -->
              <div v-if="item.child.length > 0" class="comment-child-list">
                <transition-group name="comment-child">
                  <div
                    v-for="items in item.child" :key="items.id" class="comment-child-item"
                    :data-id="items.id"
                  >
                    <div class="head">
                      <div class="img">
                      ...
                      <div class="name">
                        <a :class="{ 'admin-mark': items.admin }">{{ items.name }}<span
                          v-if="items.admin" class="mark-tit"
                        >这里有我，并且不止有我</span></a>
                        <div class="r">
                          <div class="reply" @click="reply(item, 2, items)">
                            reply
                          </div>
                    ...
                    <div class="comment-child-content">
                      <span v-if="items.type === 3" class="reply-name"> @{{ items.reply_name }}
                      </span>
                      <p>{{ items.content }}</p>
                    </div>
                  </div>
    ...
    <div>
      <PuzzleVerification :on-success="verifyResult" :verification-show="isVerification" @clone="verifyResult" />
    </div>

    <div class="admin-popup" :class="adminPopupStatus">
      <span class="iconfont icon-close2" @click="verifyPopup('admin', false)" />
      <img :src="data.base.admin_avatar">
      <div>
        哇哦～恭喜你，发现了一个小彩蛋～～
      </div>
      <input v-model="adminCode" type="text" placeholder="请输入管理员身份标识码">
      <p v-if="status == 3" class="hint">
        <span class="iconfont icon-error" />
        <span>{{ hint[status] }}</span>
      </p>
      <button @click="verifyResult('admin')">
        确定
      </button>
    </div>
  </div>
